...
exports.createOrGetAOneOnOneChat = async (req, res, next) => {
  try {
    ...
    const combinedUUIDs = req.user.id + receiver_id + cart_id
...
exports.getAllChats = async (req, res, next) => {
  try {
    ...
    const receiver = await chat_room.findAll({
      ...
      group: ['chat_room.room_id', 'Vendor.id', 'chat_data.id', 'cart.id', 'cart->vendor_service.id', 'cart->vendor_service->sub_category.id']
    })
    ...
    const unreadCounts = await db.chat_room.findAll({
            ...
            attributes: [
                'room_id',
                [
                    db.sequelize.fn('COUNT', db.sequelize.literal('CASE WHEN "chat_data"."send_from" = \'Vendor\' AND "chat_data"."unread" = true THEN 1 END')),
                    'unread_count'
                ]
            ],
            include: [
...
exports.getAllMessages = async (req, res, next) => {
  try {
    ...
    if (selectedChat.customer_id !== req.user.id) {
      return res
        .status(400)
    ...
    return res.status(200).json({
      status: true,
      message: 'Successful',
      data: { messages: messages, status: status?.booking?.status, service_name: selectedChat?.cart?.vendor_service?.sub_category?.name, vendor: selectedChat?.Vendor }
    })
  } catch (err) {
    next(err)
  }
...
exports.sendMessage = async (req, res, next) => {
  try {
    ...
    const message = await chat_data.build({
      ...
      content: content,
      time_stamp: Date.now(),
      send_from_customer: req.user.id,
      send_from_vendor: selectedChat.vendor_id,
...
exports.deleteChatById = async (req, res, next) => {
  try {
    ...
    delete_chat.customer_deleted = true
