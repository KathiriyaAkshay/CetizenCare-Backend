...
async function authenticateVendor (req, res, next) {
  ...
  try {
    const isBlacklisted = await blacklisted_token.findOne({ where: { token } })
    ...
    const user = await Vendor.findOne({ where: { id: decoded.userId } })
    if (!user) {
      return res
        .status(404)
        .json({ status: false, message: 'Vendor not found.' }) // User not found
    }
